...
export const refineFeedbackStatement = async (statement: string): Promise<string> => {
  const prompt = `
    ...
    Пользователь предоставит черновик своей обратной связи.
    Твоя задача:
    ...
    3. Проанализировать, содержит ли высказывание информацию о стандарте или нормах поведения, названа ли разница между стандартов и фактическим поведением сотрудника.
    4. Если на отклонение от стандарта/норматива не указано, перепиши высказывание так, чтобы оно содержало явное указание на стандарт и на факты отклонения от стандарта.
    5. Кратко объясни, почему были сделаны изменения и какова их польза для повышения эффективности обратной связи.
    
    Черновик пользователя: "${statement}"
    
    Отвечай на русском языке. Будь краток и полезен. Используй Markdown.
  `;
...
export const suggestGrowQuestions = async (context: string, stage: string): Promise<string> => {
  const prompt = `
    ...
    Отвечай на русском языке. Оформи в виде маркированного списка. Используй Markdown.
  `;
...
export const analyzeGap = async (data: any): Promise<string> => {
  const prompt = `
    ...
    У нас есть данные для анализа рабочего поведения сотрудника.
    ...
    Твоя задача:
    1. Сформулируй одним предложением четкую разницу между ожидаемым поведением сотрудника и его реальным поведением.
